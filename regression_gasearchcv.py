...
def read_kosovo(seed=None):
    ...
    plt.figure()
    plt.plot(
        list(data["DissolvedOxygen"][:thresh])
        + [None for i in range(thresh, len(data))],
        "b-",
        label="Data used",
    )
    plt.plot(
        [None for i in range(thresh)] + list(data["DissolvedOxygen"][thresh:]),
        "r-",
        label="Data discarded",
    )
    plt.legend()
    ...
    data.index = [
        pd.to_datetime(i.split(" 00")[0]) for i in data["Timestamp as DateTime"]
    ]
    ...
    X.describe().to_latex(
        buf=ds_name.lower() + "-describe.tex",
        caption="To in inserted",
        label="tab:wq-desc",
    )
        ...
        for dataset in datasets:
            ...
            for tk, tn in enumerate(dataset["target_names"]):
                    ...
                    (
                        ...
                        LGBMRegressor(random_state=random_seed),
                        {
                            ...
                            "learning_rate": Continuous(
                                0.001, 0.1, distribution="log-uniform"
                            ),
                ...
                for est_name, estimator, param in estimators:
                    ...
                    plt.legend()
                    plt.savefig(
                        f"{path}{BASENAME}_{run:02d}_{est_name}_{target}.png",
                        bbox_inches="tight",
                    )
                    plt.show()
